...
| 8 | ✅ RESOLVED (2026-06-30) — **the vestigial demo gate was retired.** Deleted `middleware/gate.py`, removed `DEMO_PASSWORD` (config) + the dead `make_token()` headers in `mcp_mount`/`agent`; the MCP mount now resolves identity directly from the DB (works under `REQUIRE_AUTH`); standalone `mcp_server.py`/`mcp_remote.py` fixed to JWT username/password login. API auth is now a clean 2-mechanism story: **JWT (users) or API key (services)** on `Authorization: Bearer`, plus `X-User-Id`, enforced by `REQUIRE_AUTH`. (Note: `auth_service` password check is still a placeholder — no bcrypt; fine for demo creds where password==username.) | `auth.py`, `config.py`, `mcp_mount.py`, `agent.py` |
