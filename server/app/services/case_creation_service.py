"""Create a PayGuard post-pay case from a raw X12 835 (ERA).

This is the shared core behind two entry points:
  • POST /api/analyze/835                (paste raw EDI)            — routes/analyze.py
  • POST /api/file-intake/upload (835)   (drop an .835/.x12 file)   — routes/file_intake.py

Keeping the logic here means the two paths can't drift. The function parses
the 835, resolves/creates member + provider, writes the ERA transaction +
claim + lines + case + likelihood prior, runs the detectors, recomputes
priority, and runs the soft-failing LLM FWA pass — identical to the original
analyze_835 endpoint behaviour (single claim per 835: parsed.claims[0]).
"""
...
@dataclass
class Created835:
    """Result of creating a case from an 835 — the full CaseDetail for the
    paste endpoint plus the raw IDs the file-intake flow needs to attach a
    document and record the result."""
    detail: CaseDetail
    case_id: str
    case_sequence: int
    claim_id: str
...
async def create_case_from_835(db: AsyncSession, raw_edi: str) -> Created835:
    ...
    """
    # 1. Parse
    log.info("create_case_from_835 received %d chars | first_30=%r", len(raw_edi), raw_edi[:30])
    try:
        parsed: Parsed835 = parse_835(raw_edi)
    ...
    for svc in p_claim.svc_lines:
        ...
        db.add(ClaimPayment835(
            ...
            service_date=svc.service_date or p_claim.service_date or parsed.payment_date,
        ))
    ...
    for idx, svc in enumerate(p_claim.svc_lines, start=1):
        db.add(ClaimLine(
            ...
            line_number=idx,
            cpt_code=svc.cpt_code,
            service_date=svc.service_date or service_date,
